...
const ChaoticBackground: React.FC<ChaoticBackgroundProps> = ({ speedMultiplier = 1.0 }) => {
  ...
  }, [renderedParticles, speedMultiplier]);
      ...
      {renderedParticles.map((p) => (
        <div key={p.id} ref={(el) => { if (el) p.element = el; }} className="absolute top-0 left-0 will-change-transform" style={{ width: `${p.width}px`, height: `${p.height}px` }}>
          <img
            src={p.src}
            alt=""
            className="w-full h-full object-cover rounded-xl opacity-20 brightness-[0.4] transition-all duration-1000"
          />
        </div>
      ))}
    </div>
